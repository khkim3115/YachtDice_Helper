...
트레이 아이콘을 누르면 **트레이 바로 위에 테두리 없는 팝업 패널**로 게임이 뜨고, 패널 바깥을
클릭하면 숨겨집니다(작업표시줄에는 표시되지 않음). 게임과 헬퍼 데이터(`V.bin`)를 앱에
내장(`app://` 커스텀 스킴)하므로 **인터넷 없이도** 동작합니다.
...
## 동작 메모
- 게임은 트레이 위에 뜨는 **테두리 없는 팝업 패널**(약 600×880, 화면 높이에 맞춰 보정).
- **패널 바깥 클릭(포커스 상실) → 자동 숨김.** 작업표시줄에는 표시되지 않음(`skipTaskbar`).
- 패널 위치는 트레이 아이콘 기준으로 매번 우하단(작업 영역 안)에 다시 계산해 배치.
- 완전 종료는 트레이 메뉴 **종료**로만(닫기/숨김은 종료가 아님).
- 단일 인스턴스 — 이미 떠 있으면 기존 패널을 보여줍니다.

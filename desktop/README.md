...
**트레이 위에 작은 팝업**이 떠서 그 안에서 바로 게임을 합니다(별도 큰 창 없음). 게임과 헬퍼
...
- **플레이** 선택 시 트레이 위에 **작은 팝업(약 440×680, 화면 높이에 맞춰 보정)** 이 떠서
  그 안에서 게임을 플레이. 작은 크기에 맞게 화면을 `zoomFactor 0.8` 로 축소.
- **팝업 바깥 클릭(포커스 상실) → 자동 숨김.** 작업표시줄에는 표시되지 않음(`skipTaskbar`).
- 팝업 위치는 트레이 아이콘 기준으로 매번 우하단(작업 영역 안)에 계산해 배치.
- 완전 종료는 메뉴 **종료**로만(닫기/숨김은 종료가 아님). 팝업은 미리 로드해 첫 오픈이 빠름.

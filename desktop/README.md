...
- **Windows 시작 시 자동 실행** — 부팅 시 트레이 상주 토글 (macOS 에선 "로그인 시 자동 실행")
- **라이트 모드** — 다크/라이트 테마 토글
- **위치 고정** — 켜면 팝업이 바깥 클릭에도 숨지 않음(다른 작업 하며 띄워 두기)
- **위치 초기화** — 드래그로 옮긴 팝업을 트레이 우하단 기본 위치로 복귀(옮긴 적 있을 때만 활성)
...
- **팝업 바깥 클릭(포커스 상실) → 자동 숨김**(멀티 중에도 동일 — 창은 파괴되지 않고 숨겨지므로
  실시간 구독이 유지되어 다시 열면 최신 상태). 단, 트레이 메뉴 **위치 고정**을 켜면 바깥 클릭에도 숨지 않습니다(`settings.pinned` 저장).
  작업표시줄에는 표시되지 않음(`skipTaskbar`).
- 팝업 **헤더를 끌어** 원하는 위치로 옮길 수 있고(버튼·입력은 클릭 유지), **옮긴 위치를 기억**해 다음 표시 때 그 자리
  (작업 영역 안으로 클램프)에 복원합니다. 옮긴 적이 없으면 트레이 아이콘 기준 우하단에 배치하고, 트레이 메뉴 **위치 초기화**로 기본 우하단으로 되돌립니다.
